(function () {
    ...
    function connectWebSocketTerminal() {
        ...
        terminal.write('Connecting to WebSocket terminal...\r\n');
        ...
        ws.onopen = function () {
            ...
            terminal.write('Connected to WebSocket terminal\r\n');
    ...
    function onMQTTConnect() {
        ...
        terminal.write('Connected to MQTT terminal\r\n');
    ...
    // 配置Shell环境
    function sendShellConfiguration() {
        // 发送常用的Shell配置命令
        var commands = [
        ...
        for (var i = 0; i < commands.length; i++) {
            if (communicationMode === 'mqtt') {
                sendMQTTCommand('input', mqttSessionID, commands[i] + "\n");
            } else {
                // WebSocket模式
                var encoder = new TextEncoder();
                var uint8Array = encoder.encode(commands[i] + "\n");
                ws.send(uint8Array);
            }
            ...
            if (i < commands.length - 1) {
                setTimeout(function(cmd) {
                    return function() {
                        if (communicationMode === 'mqtt') {
                            sendMQTTCommand('input', mqttSessionID, cmd + "\n");
                        } else {
                            // WebSocket模式
                            var encoder = new TextEncoder();
                            var uint8Array = encoder.encode(cmd + "\n");
                            ws.send(uint8Array);
                        }
                    };
                }(commands[i+1]), (i+1) * 100);
            }
        }
    }
